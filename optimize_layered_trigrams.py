...
def cv_optimize(clf, parameters, X, y, n_jobs=-1, n_folds=5, score_func=None):
    if score_func:
        gs = GridSearchCV(clf, verbose=1, param_grid=parameters, cv=n_folds, n_jobs=n_jobs, scoring=score_func)
    else:
        gs = GridSearchCV(clf, verbose=1, param_grid=parameters, n_jobs=n_jobs, cv=n_folds)
    gs.fit(X, y)
    return gs
...
#     parameters = {'n_estimators':[100,300,500,1000], 'max_features':['auto','sqrt','log2'],'criterion':['gini','entropy'], 'max_depth':[None]+range(0,5), 'min_samples_leaf':[1,2,3]}

#     rf = RF(class_weight="balanced", oob_score=True, n_jobs=-1)
#     gs = cv_optimize(rf,parameters,X_train_train,y_train_train)
#     print "done learning"
...
if __name__ == "__main__":
    ...
    rf = RF(class_weight="balanced", oob_score=True)
    gs = cv_optimize(rf,parameters,X_train_train,y_train_train)
    print "done learning"
    print gs.best_estimator_.score(X_test_train,y_test_train)
    pd.DataFrame(gs.cv_results_).to_csv("optimize.csv")
